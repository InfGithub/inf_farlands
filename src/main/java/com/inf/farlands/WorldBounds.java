 ...
 *   Y:        [worldGenMinY, worldGenMaxY)（半开——worldGenMaxY 是缓冲带起始）
 *
 * 缓冲带（不可用于正常逻辑，各 16 格 = 恰好一个整 chunk）：
 ...
 * （等价位对称定义 [~P, P]，P = Integer.MAX_VALUE - 16 = MAX_BLOCK - 1）。
...
public final class WorldBounds {
    ...
    /** 中间值可能已溢出 int（结构 boundingBox 等），long 版承接。 */
    ...
    /** 范围 [minY, maxY) 是否完全在可玩高度内（hasChunksAt 语义）。 */
