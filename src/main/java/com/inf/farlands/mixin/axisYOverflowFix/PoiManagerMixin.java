 ...
 * 遍历 section 范围固定为 levelHeightAccessor 的 [-4, 20)，该范围由
 * LevelHeightAccessorMixin 对 ServerLevel 固定。极端 Y 时 section 约 1.34 亿，
 * POI 查不到；数据存在、存储正常，只是查找范围不含。
 *
 * 修复：@Overwrite getInSquare——Y 遍历范围 = 维度范围与查询位置附近。
 * 正常 Y 时 pos 范围被 distinct 吸收，与 vanilla 完全等价，PortalForcer
 * 传送门查找不受影响；极端 Y 时覆盖极端 section，村民查到 POI。距离过滤
 * |dx|/|dz| 保持 vanilla。维度范围硬编码 -4/20，与 getInChunk 的
 * levelHeightAccessor getMinSection/getMaxSection 运行时值一致，由
 * LevelHeightAccessorMixin 固定。
 *
 * getOrLoad 是 SectionStorage 的 protected 继承方法，@Shadow 不解析继承方法，
 * 所以走反射。不能用 getOrLoad：对极端 Y 未预填充的无条目 section 调用
 * readColumn 读磁盘后仍无条目，于是抛 IllegalStateException——vanilla 假设
 * 维度范围内必有条目。改用 get，storage 直查，无条目返回 null 不抛：
 * 正常 Y 等价，readColumn 预填充维度范围 empty；极端 Y 覆盖内存 POI，
 * 刚 add 的必在 storage。
 */
