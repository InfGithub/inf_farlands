 ...
 *
 * level.isOutsideBuildHeight 被本 mod 覆盖为 Config 语义 ±2.14B →
 ...
 * level.getSectionIndexFromSectionY，vanilla 索引 y+4，取窗口数组 34 section
 * → 系统性错位；改用 chunk.getSectionIndexFromSectionY，窗口索引。
 *
...
public abstract class ClonedChunkSectionCacheMixin {
    ...
    private boolean wrapIsOutsideBuildHeight(Level instance, int y, Operation<Boolean> original,
        ...
        // blockY，必须 >>4 转回 sectionY，16 的倍数，算术右移无精度损失；否则窗口检查
        // 用错单位：窗口内 section 被误拦截，变空气，只渲染 y≈0；窗口外 section 漏拦截，
        // 越界 CTD。
        int sy = y >> 4;
