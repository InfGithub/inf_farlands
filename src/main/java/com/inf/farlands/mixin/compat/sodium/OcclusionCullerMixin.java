 ...
 *
 * 冲突（OcclusionCuller.init）：用 level.getMinSection()/getMaxSection()
 * （vanilla -4/20）判断玩家 sectionY 是否在"世界高度内"。本 mod 允许玩家
 * 在维度外（Config 语义 ±2.14B）——玩家在 y=2.14B 时 sectionY=134M >= 20
 * → outOfWorld 路径（只渲染 y=19 水平层）→ 2.14B 的 RenderSection 永不
 * 被收集/编译 → setblock 不渲染。
 *
 * 适配：init 的高度判断改窗口范围（玩家所在 chunk 的窗口）。玩家 = 相机
 * （单机），与 viewport.getChunkCoord 一致；第三人称/spectator 的相机偏差
 * 在窗口 ±17 容差内，可接受。
 *
